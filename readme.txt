...
The camera script was adapted from (https://bit.ly/2NnaWby). Changes in mouse xy coordinate are converted to changes in pitch and yaw. Further changes (improvements?) include persistent turning when the mouse is outside the screen, as well as the addition of a Rigidbody component so that the camera does not phase through terrain. WASD movement therefore is applied through forces rather than direct translation.
